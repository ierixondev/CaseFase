// -------------- CONFIG --------------
// URL вашего Node.js сервера (где будут api и sqlite)
const SERVER_URL = "https://YOUR_SERVER_URL"; // <-- Замените на реальный публичный URL, напр. https://my-app.example.com
// -------------------------------------
...
langSelect.addEventListener("change", e=>{
  LANG = e.target.value;
  applyTexts();
  // optionally persist to server (not required)
});
...
buyBtn.addEventListener("click", async ()=>{
  ...
  try{
    ...
    if(res && res.ok && res.invoiceLink){
      ...
      // После оплаты бот (getUpdates) обновит DB; здесь мы будем опрашивать профиль
    } else {
...
async function refreshProfile(){
  ...
  try{
    ...
    if(res && res.ok && res.profile){
      ...
      // update name in case server has username
